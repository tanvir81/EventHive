    ...
    <div className="w-full bg-white rounded-2xl border border-gray-100 shadow-xl shadow-gray-100/50 overflow-hidden">
      ...
      <div className="p-6 border-b border-gray-100 flex items-center justify-between bg-white">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">Manage and track your listed events</p>
        </div>
      ...
      {events.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-20 text-center">
          ...
          <p className="text-gray-500 text-sm mt-1 mb-6">Start by creating your first event now.</p>
          <Link to="/dashboard/add-event" className="px-5 py-2.5 bg-black text-white text-sm font-medium rounded-xl hover:bg-gray-800 transition-all">
            Create Event
          </Link>
        </div>
      ) : (
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-gray-100">
              {events.map((event, index) => (
                <tr key={event._id} className="group hover:bg-gray-50/50 transition-colors duration-200">
